...
## Current status: Day 9 - the parallel group is parallel, and requests are traced
...
- `scripts/` - dev tooling, not shipped code. `runlog.py` records any run as structured JSON under `test-results/`. Seven live scripts, each costing real tokens and each opt-in: `check_structured_output.py` (per-model contract validity), `check_day5_checkpoint.py` (chaos injected -> agent JSON, scored against ground truth), `check_agent_selection.py` (coordinator routing; defaults to 2 of 5 cases), `check_day7_delegation.py` (plan -> execute end to end, 2 calls; asserts the agent's wire prompt is exactly `context_passed` + query, and that a coordinator-only sentinel does not leak), `ingest_embeddings.py` (Voyage; embeds new/stale corpus rows, `--dry-run` is free), `check_day8_docs.py` (1 call; the Docs agent answers from the seeded corpus with verbatim citations), and `check_day9_trace.py` (~3 calls, or zero with `--fake-agents`; one Langfuse trace showing two agents running concurrently - needs the Langfuse keys).
