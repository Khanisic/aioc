...
## Current status: Day 11 - the GitHub agent, the first agent on the MCP wire

The repository is at the end of Day 11.
...
Day 8 added the retrieval layer (`src/aioc/retrieval/`: pgvector + pg_trgm hybrid search with RRF fusion, Voyage embeddings behind an `Embedder` protocol, lexical-only degradation when no key is set, `docker/postgres/init/04-embeddings.sql`) and the Docs agent (`src/aioc/agents/docs.py`: retrieval-grounded, every claim cited with a verbatim quote, hallucinated sources rejected in code), registered in the executor's `default_runners()`.
Day 9 made the executor's parallel group actually concurrent (a thread pool, per-runner `Usage` accumulators merged after the join, results in plan order, a barrier test proving overlap offline) and added the tracing seam (`src/aioc/observability/tracing.py`: `Tracer`/`RequestTrace`/`AgentSpan` protocols, `NullTracer` default, a lazily-constructed `LangfuseTracer` adapter; one trace per request with a `plan` span and one `agent:<name>` span per invocation, `CoordinatorResponse.trace_id` filled from it). Tracing is opt-in at the entry point - only code that passes a tracer (`default_tracer()`) ever emits a span, so the offline suite stays network-free even with Langfuse keys in `.env`.
Day 10 proved the whole pipeline live (`scripts/demo_day10.py`: inject chaos, render live Prometheus metrics as the situation block, then `respond()` end to end with tracing). Two measured runs, both diagnosing the injected truth: the canonical query ran Incident alone because dynamic selection judged it needed no docs lookup (that judgment beating the demo author's prediction is the graded behaviour working), and the showcase query ran Incident + Docs in parallel - two agents for the wall-clock price of one - with 7 claims cited across 4 corpus documents. The checkpoint GIF (`docs/assets/day10-demo.gif`) is rendered from the run's recorded transcript by `scripts/render_demo_gif.py`; numbers live in `docs/interview-prep/numbers.md`.
Day 11 added the GitHub agent (`src/aioc/agents/github.py`) and the first agent-consumed MCP server: `src/aioc/tools/github/` is a real stdio server (`get_pull_request`, `list_commits`, `diff_refs`) over the GitHub REST API with the four-class error mapping and keys-only patch redaction, `src/aioc/llm/mcp.py` (`McpStdioToolset`) launches it as a subprocess and exposes its tools as harness `ToolSpec`s, and the agent drives them in `run_tool_loop` before being forced through `emit_github_report`. Factual fields (titles, SHAs, counts) are stamped from the tool output rather than asked of the model; every PR, commit, `change_ref`, and excerpt must trace back to a tool reply or the agent raises. Registered in `default_runners()`, so github queries no longer produce an `agent_not_implemented` gap.
Still downstream: the Deployment agent (Day 12), the refinement loop and model-written synthesis (Day 14), and the eval harness (Day 19).
...
- `src/aioc/llm/` - the Claude API harness (Day 2, Reasoning Layer): `LLMClient` with `complete` (messages), `stream_text` (streaming), and `run_tool_loop` (a manual `tool_use` loop with per-call `ToolCallRecord` audit records); `ToolSpec`/`ToolResult`; `LLMSettings` via pydantic-settings.
  ...
  `mcp.py` (Day 11) is `McpStdioToolset`: a stdio MCP server as `ToolSpec`s, its session on a dedicated thread so sync agents (possibly on the executor's worker threads) can call it.
...
- `src/aioc/agents/` - Reasoning Layer subagents. `incident.py` is live (Days 3-4): `IncidentAgent.investigate` returns prose; `IncidentAgent.diagnose` returns a schema-validated `IncidentAgentResponse`, forcing the model through a single structured-output tool whose JSON Schema is generated from the frozen models and then annotated with the contract's cross-field rules (`_apply_guidance`).
  ...
  Descriptions are added, never shape, and `aioc.contracts` stays untouched; the guard raises at import if a field it annotates no longer exists (the shared helper is `_annotate.py`).
  `github.py` is live (Day 11): `GitHubAgent.analyze` opens the `aioc-github` MCP server through the injectable `Toolset` seam, lets the model investigate with its three tools, then forces `emit_github_report`; facts are stamped from the ledger of tool replies, ungrounded references raise `GitHubAgentError`, and each wire call becomes a contract `ToolCallRef`.
  `docs.py` is live (Day 8): `DocsAgent.answer` retrieves from the corpus first (through the injectable `CorpusRetriever` seam), renders the documents into the prompt, and enforces grounding in code - a cited `document_id` retrieval never returned or a paraphrased quote raises `DocsAgentError`, and the `Coverage` counters are stamped by the runtime rather than asked of the model.
  Deployment lands on Day 12.
- `src/aioc/coordinator/` - the orchestrator. `planner.py` is live (Day 6): `Coordinator.plan` returns a validated `SelectionPlan` (intent, selected agents with their explicit context, skipped agents with reasons). The graded Domain 1 behaviours are enforced by validators, not prompt text - all four agents must be accounted for, `depends_on` must resolve inside the plan (acyclically), and `context_passed` must be non-empty *and* not merely restate the query. Reads `AIOC_COORDINATOR_MODEL`, which is the seam the Day 23 routing experiment measures. The model is asked for `PlannedInvocation` (`AgentInvocation` minus `round`) and the coordinator stamps `round` itself - plumbing it already knows is a field the model can only get wrong, which it demonstrably did.
  `executor.py` is live (Day 7, parallel + traced Day 9): `Executor.execute` consumes a plan into a contract `CoordinatorResponse`, handing each runner exactly `context_passed` and nothing else; an invocation it cannot run (only Deployment is missing until Day 12) becomes a `Gap` with `resolvable: false`, never a fabricated response. The parallel group runs on a thread pool with per-runner `Usage` accumulators merged after the join (a shared `+=` from two threads would lose token counts silently); the sequential chain stays ordered and results merge in plan order. Synthesis is deterministic until Day 14; `cost` is accumulated from `Usage`, never estimated. `respond()` glues plan + execution and owns the request trace (a `plan` span, then the executor's agent spans). New agents register in `default_runners()`. The refinement loop is Day 14.
- `src/aioc/tools/` - Platform Layer MCP tools, grouped `incident/`, `github/`, `docs/`, `deployment/`. `envelope.py` is the shared contract wire envelope (sec 6); `incident/timeline_server.py` (`get_incident_timeline`, Day 6) and `incident/correlate_server.py` (`correlate_events`, Day 7) are real stdio MCP servers reading the seeded corpus, with connection settings shared in `incident/store.py`. `github/api.py` + `github/server.py` (Day 11) are the GitHub REST client and the `aioc-github` stdio server; `GITHUB_SCOPE_MISSING` is the `permission` class for a missing or under-scoped token, and patches come back with `KEY=value` lines redacted to the key. `policy.py` is the chaos ground-truth gate: any `chaos*` service returns a `permission` error (`CHAOS_SCOPE_REQUIRED`) from every server, because those signals are the Day 19 eval's answer key. **None of these import `aioc.contracts`** - the MCP boundary is JSON Schema (contract sec 6), so enums and input schemas are longhand, with a test asserting the copies still match the Python enums. Framework input validation is deliberately off: it returns plain text where the contract requires a structured `validation` error.
- `src/aioc/retrieval/` - the Day 8 retrieval layer: `embeddings.py` (the `Embedder` protocol, the Voyage client, `default_embedder()` returning `None` without a key) and `corpus.py` (ingestion with sha256 stale detection into `incident_embeddings`, hybrid pg_trgm + pgvector search fused with RRF, honest lexical-only degradation recorded in `RetrievalResult.degraded`).
  The embedding model choice (`voyage-3.5`, 1024 dims) is pinned by the `vector(1024)` column in `04-embeddings.sql`; the `model` column marks which vectors are comparable, and re-embedding never rewrites the corpus.
- `src/aioc/observability/` - both directions of one concern. `prometheus.py` is live (Day 5): a thin PromQL client plus `build_incident_context`, which renders live demo-app metrics as the Incident agent's explicit context block. **`chaos_knob_value` is excluded by an enforced guard** at both the query and the rendered-output level - it is the Day 19 eval's ground truth, and leaking it would make evals pass silently. `tracing.py` is live (Day 9): the `Tracer`/`RequestTrace`/`AgentSpan` seam, `NullTracer` (the default everywhere - tracing is opt-in at the entry point so the offline suite stays network-free), and the `LangfuseTracer` adapter (client built lazily, injectable for tests). `ToolCallRef` records ride as child events whose metadata carries their measured timing, because the SDK stamps observation timestamps at creation.
...
- `tests/test_executor.py` - Day 7 delegation, driven by fake runners through the `AgentRunner` protocol. The two done-when facts have direct tests: the runner receives exactly `context_passed` (nothing inherited, nothing enriched), and an unimplemented agent yields a `resolvable: false` `Gap` rather than a fabricated response. Also dependency ordering, failure isolation, and measured cost. Day 9 added the concurrency section: a barrier test where each parallel runner blocks until the other arrives (serial execution fails it), plan-order determinism, per-runner accumulator isolation, and fake-tracer assertions on spans, tool-call refs, and `trace_id`.
- `tests/test_docs_agent.py` - Day 8 Docs agent with a scripted fake client and a fake retriever: explicit context passing, document rendering, the grounding rejections (unretrieved source, paraphrased quote), stamped coverage accounting, and the new sec 4.2 unanswered-needs-a-Gap negative test (its contract-level twin lives in `test_contract.py`).
- `tests/test_retrieval.py` - Day 8 retrieval: rendering, stale detection, RRF fusion, and the Voyage client offline (httpx `MockTransport`); `integration`-marked hybrid tests run ingestion with a fake embedder inside a rolled-back transaction, so the shared database is never mutated.
- `tests/test_tracing.py` - Day 9 tracing: null-object degradation without keys, the Langfuse adapter's observation tree against an injected stub client (never the real SDK client - offline means no OTel export machinery either), error marking, and the `.env` path regression pinned the same way `test_retrieval.py` pins it.
- `tests/test_github_agent.py` - Day 11 GitHub agent with a scripted fake client and an in-process fake toolset: the two-phase loop, context passing, fact stamping, the grounding rejections (unfetched PR/commit/change_ref, paraphrased excerpt), error-class recording, and the no-data-is-not-complete rule.
- `tests/test_github_tool.py` - Day 11 GitHub tools against an httpx `MockTransport`: envelope and meta, all four error classes from real paths, rate-limit and timeout mapping, keys-only redaction, output bounds, the description template, no-contracts-import, and enum drift.
- `tests/test_mcp_toolset.py` - Day 11 MCP client seam over the real stdio wire (the server subprocess with a blanked token, so no network): listing, `isError` propagation, cross-thread calls, startup failure.
...
- `scripts/` - dev tooling, not shipped code. `runlog.py` records any run as structured JSON under `test-results/`. Seven live scripts, each costing real tokens and each opt-in: `check_structured_output.py` (per-model contract validity), `check_day5_checkpoint.py` (chaos injected -> agent JSON, scored against ground truth), `check_agent_selection.py` (coordinator routing; defaults to 2 of 5 cases), `check_day7_delegation.py` (plan -> execute end to end, 2 calls; asserts the agent's wire prompt is exactly `context_passed` + query, and that a coordinator-only sentinel does not leak), `ingest_embeddings.py` (Voyage; embeds new/stale corpus rows, `--dry-run` is free), `check_day8_docs.py` (1 call; the Docs agent answers from the seeded corpus with verbatim citations), and `check_day9_trace.py` (~3 calls, or zero with `--fake-agents`; one Langfuse trace showing two agents running concurrently - needs the Langfuse keys). Day 11 added `check_day11_github.py` (~3-5 calls; the GitHub agent reads a real PR over the MCP wire). Day 10 added `demo_day10.py` (~3 calls; the full inject -> metrics -> `respond()` story, recording a timed transcript) and `render_demo_gif.py` (free; renders that transcript into the checkpoint GIF, pillow via PEP 723 inline metadata).
...
- `docker/postgres/init/` - runs once, on first initialisation of an empty Postgres volume, in alphabetical order: `01-extensions.sql` (vector, pg_trgm, pgcrypto), `02-incidents.sql` (the incident corpus schema), `03-seed-incidents.sql` (18 synthetic incidents, 65 timeline events), `04-embeddings.sql` (the `incident_embeddings` table; additive and `IF NOT EXISTS`, so `scripts/ingest_embeddings.py` applies it to an already-initialised database without a reset). Changing one of the first three files needs `make db-reset` (destructive) to take. Note `init/` never runs against hosted Postgres, so Day 24 applies these files with `psql` - each is written to be runnable standalone and the seed is idempotent.
