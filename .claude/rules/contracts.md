...
`src/aioc/contracts/` is jointly owned - the one package both engineers import.
...
- Changing a frozen thing requires the CONTRACTS.md sec 0 process: written agreement, a
  `schema_version` bump (patch = additive-optional, minor = additive-required or new enum member,
  major = removal or type change), and a changelog row in sec 9.
